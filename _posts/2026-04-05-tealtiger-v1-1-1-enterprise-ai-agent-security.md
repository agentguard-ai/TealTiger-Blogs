...
description: "A deep dive into TealTiger v1.1.1 — the open-source AI agent security SDK with policy enforcement, guardrails, circuit breakers, audit logging, and 7-provider coverage."
tags: [release, enterprise, security, guardrails, policy, audit, multi-provider, owasp]
---
...
![TealTiger Platform Architecture](/assets/images/blog/01-platform-architecture.png)
...
![Request Lifecycle](/assets/images/blog/02-request-lifecycle.png)

Every step produces a typed `Decision` object with a consistent contract — action, reason codes, risk score, and correlation ID. Your application logic can handle any outcome uniformly, regardless of which component triggered it.
...
![Policy Rollout Modes](/assets/images/blog/03-policy-rollout-modes.png)
...
![TealGuard Parallel Execution](/assets/images/blog/04-guardrail-parallel.png)
...
![TealMonitor Anomaly Detection](/assets/images/blog/10-monitoring-anomaly.png)
...
![TealCircuit State Machine](/assets/images/blog/05-circuit-breaker.png)
...
![TealAudit Redaction Levels](/assets/images/blog/06-redaction-levels.png)
...
![Multi-Provider Coverage](/assets/images/blog/07-multi-provider.png)
...
![End-to-End Traceability](/assets/images/blog/09-traceability.png)
...
![OWASP Coverage Map](/assets/images/blog/08-owasp-coverage.png)
...
TealTiger includes a built-in policy test harness that validates policy behavior before production deployment.
...
Each test executes in under 100ms. Results are deterministic and reproducible.
...
const audit = new TealAudit({
  ...
  config: { input_redaction: RedactionLevel.HASH, detect_pii: true }
});

const client = new TealOpenAI({
  apiKey: process.env.OPENAI_API_KEY,
  agentId: 'my-agent',
  engine, guardrailEngine: guardrails, audit
});
```
...
engine = TealEngine(
    policies={"tools": {"database_query": {"allowed": True}, "file_delete": {"allowed": False}}},
...
client = TealOpenAI(
    api_key="your-key", agent_id="my-agent",
    engine=engine, guardrail_engine=guardrails, audit=audit
)
```
...
TealTiger v1.1.1 is available now on [npm](https://www.npmjs.com/package/tealtiger) and [PyPI](https://pypi.org/project/tealtiger/).
...
- [TypeScript SDK (npm)](https://www.npmjs.com/package/tealtiger)
- [Python SDK (PyPI)](https://pypi.org/project/tealtiger/)
- [GitHub](https://github.com/agentguard-ai/tealtiger)
- [OWASP ASI Mapping](https://github.com/agentguard-ai/tealtiger/blob/main/OWASP-AGENTIC-TOP10-TEALTIGER-MAPPING.md)
